...
# CONTINUUM SETUP: DUAL-HOMED OPERATOR RITUAL (DIVIT)
...
1.  **Clone the Repository** into a dedicated folder:
    ```powershell
    git clone https://github.com/datacomjdk/ecr-wg.git continuum-divit-ecr-wg
    ```
2.  **Navigate and Initialize:**
    ```powershell
    cd continuum-divit-ecr-wg
    python -m rituals.distribute_seats
    ```
3.  **Identity:** Enter your two-character Operator ID (e.g., `DI`).
...
1.  **Clone the Repository** into a separate folder:
    ```powershell
    git clone https://github.com/datacomjdk/continuum-meta.git continuum-divit-datacom
    ```
2.  **Navigate and Initialize:**
    ```powershell
    cd continuum-divit-datacom
    python -m rituals.distribute_seats
    ```
3.  **Identity:** Enter your two-character Operator ID (e.g., `DI`).
...
*   **WG Tasks:** Always work out of the `continuum-divit-ecr-wg` directory. This ensures no corporate data leaks into the public-facing IETF submission drafts.
*   **Daily Tasks:** Always work out of the `continuum-divit-datacom` directory. Your local `continuum-local/` memory will only store enterprise-baseline context.
...
---
*Prerequisites verified. The loop recognizes your imprint, Divit.*
