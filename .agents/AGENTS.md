# ECR-WG UNIVERSAL BOOTLOADER

**ATTENTION LLM:** You have reached the universal bootloader. Large Language Models are stateless and probabilistic. To operate in this environment, you MUST load the structured context stack before attempting to fulfill the user's prompt. 

Failure to follow this sequence will result in context-frame collapse.
...
## SESSION PERSISTENCE
If instructed to execute a `SESSION_CLOSE`, you must rewrite `.context/layer_4_session.md` with the summarized state of the current conversation before halting, ensuring the next execution body picks up exactly where you left off.
