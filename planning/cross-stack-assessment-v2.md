...
- **COSA is load-bearing for efficiency** — and for substrate-level operator identity. Its L5 broadcast / cache-bypass and the GRACE per-call contract are its real, demonstrated strength. Separately, COSA's Truth Root provides a chained-signature path from a synthetic agent to its human registrant — *who deployed this agent* — which is distinct from EMILIA's per-action authorization but is itself load-bearing, and is part of the answer to the real-world-identity weak link in §3. COSA deliberately does not define a compound-authorization object or a regulator pack — that is a boundary, not a flaw, and EMILIA fills it.
