...
- **`sender_signature`** -- signature by the synthetic agent's enrollment key (see [Public Trust Store](../public-trust-store.md)).
...
- **Verifiable by any overlay node.** The signature is checkable against the public Trust Store registry without contacting the originating synth.
...
1. **Signature scheme.** Tied to the [Public Trust Store](../public-trust-store.md) decision. Open.
...
- Signature scheme is specified (depends on Public Trust Store advancement).
...
- Signed against: [Public Trust Store](../public-trust-store.md)
